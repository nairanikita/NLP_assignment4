...
def create_augmented_dataloader(args, dataset):
    ...
    print("🔄 Creating augmented dataloader (original + 5k transformed samples)...")

   
    subset = dataset["train"].shuffle(seed=42).select(range(5000))

    
    transformed_subset = subset.map(custom_transform, load_from_cache_file=False)

    
    tokenized_original = dataset["train"].map(tokenize_function, batched=True)
    tokenized_transformed = transformed_subset.map(tokenize_function, batched=True)
    ...
    augmented_dataset = concatenate_datasets([tokenized_original, tokenized_transformed])
    ...
    print(f"Augmented training dataloader ready: {len(augmented_dataset)} samples "
          f"({len(dataset['train'])} original + 5000 transformed)")
